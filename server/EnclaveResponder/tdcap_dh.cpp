...
static sgx_status_t create_cmac128(
    const sgx_ec_key_128bit_t mac_key,
    const uint8_t* data_buf,
    uint32_t buf_size,
    sgx_cmac_128bit_tag_t* mac_buf)
{
    sgx_status_t se_ret = SGX_SUCCESS;
...
static sgx_status_t dh_dcap_initiator_proc_msg1(const sgx_dh_dcap_msg1_t* msg1, sgx_dh_dcap_msg2_t* msg2, sgx_dh_session_t* sgx_dh_session)
{
    ...
    se_ret = app_derive_key(&shared_key, "SMK", (uint32_t)(sizeof("SMK") -1), &dh_smk);
    if(SGX_SUCCESS != se_ret)
    {
        goto error;
    }
    ...
    sgx_dh_session_enclave_identity_t* responder_identity)
{
    ...
    // derive AEK
    se_ret = app_derive_key(&session->initiator.shared_key, "AEK", (uint32_t)(sizeof("AEK") -1), aek);
    if(SGX_SUCCESS != se_ret)
    {
        goto error;
    }
...
error:
    // clear shared key and smk_aek
    memset(&session->initiator.shared_key, 0, sizeof(sgx_ec256_dh_shared_t));
    memset(&session->initiator.smk_aek, 0, sizeof(sgx_key_128bit_t));
    memset(session, 0, sizeof(sgx_internal_dh_session_t));
    session->initiator.state = SGX_DH_SESSION_STATE_ERROR;
                                       ...
                                       const sgx_key_128bit_t *dh_smk)
{
    ...
    if (se_ret == SGX_SUCCESS && retstatus == 0)
    {
    ...
    {
        SAFE_FREE(quote);
        return (se_ret != SGX_SUCCESS) ? se_ret : SGX_ERROR_UNEXPECTED;
    }
                                         ...
                                         sgx_dh_dcap_msg3_t *msg3)
{
    ...
    if (SGX_SUCCESS == se_ret && retstatus == 0) {
        // Successfully got QE target info
    }
    else
    {
        return (se_ret != SGX_SUCCESS) ? se_ret : SGX_ERROR_UNEXPECTED;
    ...
    if(!quote_buffer)
        return SGX_ERROR_OUT_OF_MEMORY;
    uint32_t quote_size;
    uint32_t retstatus_gen;
    se_ret = ecdsa_quote_generation_ocall(&retstatus_gen, &quote_size, &app_report, quote_buffer);
    if (se_ret == SGX_SUCCESS && retstatus_gen == 0)
    {
        // Quote generation successful
    }
    else
    {
        SAFE_FREE(quote_buffer);
        return (se_ret != SGX_SUCCESS) ? se_ret : SGX_ERROR_UNEXPECTED;
    }
...
// Function sgx_dh_dcap_responder_proc_msg2 processes M2 message, generates M3 message and makes update to the context of the session.
                                              ...
                                              sgx_dh_session_enclave_identity_t* responder_identity)
{
    ...
    //from responder private key and initiator public key
    ...
    // derive session key
    se_ret = app_derive_key(&shared_key, "AEK", (uint32_t)(sizeof("AEK") -1), aek);
    if(se_ret != SGX_SUCCESS)
    {
        goto error;
    }
